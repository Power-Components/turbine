...
use Illuminate\Database\Eloquent\{Builder as EloquentBuilder, Model, RelationNotFoundException};
...
use PowerComponents\Turbine\Support\SchemaInspector;
use stdClass;
use Throwable;
...
class SearchHandler implements SearchHandlerContract
{
    ...
    protected function getColumnList(EloquentBuilder|QueryBuilder $query, string $modelTable): array
    {
        ...
        try {
            return SchemaInspector::columnTypes($modelTable, $connection);
        } catch (Throwable) {
            return SchemaInspector::columnListing($modelTable, $connection);
        }
    }
