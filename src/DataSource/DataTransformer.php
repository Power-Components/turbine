...
use PowerComponents\Turbine\ManageLoops;
...
final class DataTransformer
{
    ...
    public function transform(BaseCollection $collection): TransformResult
    {
        ...
        $transformedCollection = $collection->map(function ($row, $index) use ($loopInstance, &$actionsByRow, $collectActions) {
            ...
            $transformedData->__turbine_loop = $loopVars;
            $transformedData->__turbine_rules = $this->component->prepareActionRulesForRows($row, $loopVars);

            if ($collectActions) {
                $processedActions = $this->actionProcessor->process($rowObject);
                $transformedData->__turbine_actions = $processedActions;
        ...
        return new TransformResult($transformedCollection, $actionsByRow);
    }
